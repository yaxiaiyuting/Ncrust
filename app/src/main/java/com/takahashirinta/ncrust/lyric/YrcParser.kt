 ...
 *
 * 但实测同时确认了另一件更强的事实 —— **yrc 与 lrc 的「行」是一一对应的**：
 *
 ...
 *
 * 所以按 **行序** 对齐：行数必须相等，且抽样校验时间戳不能差太远，否则整首放弃逐字
 * （宁可没有逐字，也不要给出错位的高亮）。
 *
...
object YrcParser {
    ...
    /** 行序对齐时允许的最大时间偏差；超过就认为两份数据不同源，整首放弃逐字。 */
     ...
     *
     * 对齐失败（行数不等 / 时间漂移过大 / 某个词定位不到）时对应行原样返回，
     * 渲染自动退化成普通 LRC 行。
     */
    fun attachWords(lines: List<LrcLine>, yrcText: String): List<LrcLine> {
        ...
        if (yrcLines.size != lines.size) {
            Log.i(TAG, "attachWords: line count mismatch lrc=${lines.size} yrc=${yrcLines.size}, skip")
            return lines
        }
        // 行序对齐的一致性抽样：时间戳差得太多的行占比过高就整首放弃。
        val close = lines.indices.count { i ->
            kotlin.math.abs(yrcLines[i].startMs - lines[i].timeMs) <= MAX_LINE_DRIFT_MS
        }
        if (close < (lines.size * MIN_ALIGN_RATIO).toInt()) {
            Log.i(TAG, "attachWords: drift too large ($close/${lines.size}), skip")
            return lines
        }
        var linesWithWords = 0
        ...
        Log.i(TAG, "attachWords: ${linesWithWords}/${lines.size} lines got word timing")
        return if (linesWithWords == 0) lines else merged
    }
