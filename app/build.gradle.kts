...
android {
    ...
    defaultConfig {
        ...
        // **第十一次按脚本定号**：三源最大值都是 32 ⇒ 本版取 33。
        // v2.1.3 修的是**用户当场报出来的文案 bug**：手机号登录的失败分支复用了
        // 扫码那条文案（sourceQrFailed =「扫码登录失败」），于是短信登录失败时界面显示
        // 「扫码登录失败」。同时按实测/码表把 20271（验证码错）、20277/20278/20450（受限）、
        // 20279（设备超限）、104604（频繁）各自分桶，不再是「一律 else」。
        versionCode = 33
        versionName = "2.1.3-gpl"
