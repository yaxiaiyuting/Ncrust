...
| **v2.1.0** | **2026-09-25** | **网易云 + QQ 音乐双音源并存，各自独立登录。**架构上引入 `MusicSourceProvider` 抽象与 `SourceRouter`（全应用唯一按音源分叉的地方）；**QQ 音乐的数字 id 用 `1L shl 62` 标志位与网易云的 id 空间结构性隔离**，因此离线缓存 key / 歌词缓存 key / 续播进度表 / 队列判重这些既有结构一个都没改、也没有任何数据迁移。QQ 取链**一次请求批量问所有档位**再按优先级挑（服务端不会自动降级），文件名只能用 `file.media_mid`（用错时服务端照样返回 purl，直到 CDN 才 404）；搜索主通道走实测稳定的旧版 GET（新版被按 IP 限流，6 次成功 1 次）。**QRC 逐字歌词**：实测 QQ 用的**不是标准 3DES**（密钥扩展 PC-2 有一处 `-27` 偏移，标准 `DESede` 解不出来），移植了该变体并用**另一份独立实现**的真实样本逐字节比对验证；`qrc=0` 的歌解出来是行级 LRC，已自动回落。双账号：QQ cookie 存独立的 `ncrust_qq_prefs`，登出一家不影响另一家。设置页新增 QQ 音乐账号卡，`SongCard` 加音源标识（只有非网易云才显示）。合规：**档位表不含 VIP 专享的 O801 轨道**（有单测钉住）、无任何 DRM 解密/解灰/试听拼接。单测 483 个通过（新增 81，含**打真实 QQ 服务端的探针**与跨实现黄金样本比对）；权限 11 条与 v2.0.2 逐条一致、签名未变；`SweepTrack` 一个字节未改 |
...
| **v2.1.1** | **2026-09-25** | **QQ 扫码登录「网络不稳定」的真根因（v2.1.0 判错了）+ 手机号登录 + 播放页音源角标。**根因不是 v2.1.0 判定的「IP 级 WAF 风控」，而是**两个代码 bug**：① `ptqrtoken` 的 `hash33` 初值抄成了 `g_tk` 的 **5381**，官方是 **0** —— 每一次轮询都被 WAF 判成畸形请求直接 403（同一 `qrsig`/同一 IP/同一 Header 的对照实验 **3/3 复现**：初值 0 → `200 + ptuiCB('66',…)`，初值 5381 → `403 + 空 body`）；② 扫码状态码 **65/67 映射反了**（65=过期、67=已扫描），用户一扫界面就报「二维码已过期」并**停止轮询**。证据是腾讯自己的生产 JS `c_login_2.js` 的 `hash33` 实现与 `ptuiCB` 的 `case` 分支，以及服务端**实际接受过**的三组 `qrsig → ptqrtoken` 向量（v2.1.0 的注释里写着「没有真实向量可钉」，所以 5381 从来没被验证过 —— 教训：**所有变体都失败时先怀疑共同的不变量，不是先怀疑环境**）。同时把轮询失败从「一律 null」改成三态：服务端明确回绝（403/空 body）累计 3 次就**停止重试并自动切网页登录**，只有真网络异常才提示「网络不稳定」——上一版会把恒定的 403 显示成网络问题并重试满 14 分钟。**新增手机号验证码登录**：微信用户没有 QQ 号、又没法同机扫「网站应用」二维码（微信不认相册里的登录码），而官方那种一键微信登录要求 appid 与**签名**在腾讯侧配对注册，fork 结构上做不到 —— 手机号是纯 HTTP、单机可完成。实测钉住三个「写错了服务端不报错」的坑：`areaCode` 必须是字符串（传数字 → `10006`）、`comm.tmeLoginMethod=3` 在 `Login` 上硬必需（在 `SendPhoneAuthCode` 上完全不需要）、假验证码返回 `1000` + 完整 35 字段骨架（**不能靠「有没有 data」判成功**）。**播放界面新增音源角标**（`歌手 · 网易云` / `歌手 · QQ 音乐`，两个音源都标）。合规：**不读短信**（验证码手输）、不存手机号、不存密码，**权限仍是 11 条、一条未新增**；`applicationId`、签名未动；单测 **525 个全绿**（新增 42）；⚠️ **扫码确认成功那一跳与手机号登录的成功路径均未实测**（前者需要真人扫码、后者需要真实短信），详见发布说明的「未验证项」 |
| **v2.1.2** | **2026-09-25** | **手机号登录的图形验证码（`20276`）走通了；v2.1.1 的 tag 从未发布，内容全在本版。**真机反馈「点发送验证码直接回 20276，界面却说这条路走不通」—— 而 `20276` **不是错误，是风控在正常工作**：官方客户端遇到它也是弹一个 WebView 让用户滑一下。本版换了个**不依赖验证页 DOM** 的做法：从 `data.securityURL` 取验证页地址（这个 key 在实测的 `104400` 响应里就在、只是空串，所以字段位置是钉死的）→ 开在 WebView 里让用户过验证 → **把 WebView 的 cookie 读出来回传**（`QqClient.musicuLogin(extraCookie=…)`）→ 重发时带上它。第 3/4 步是整个流程的关键：验证是**会话级**的、结果落在 cookie 上，而 OkHttp 与 WebView 有各自独立的 cookie store —— 少了这两步，用户看到的就是「验证完了还要再验证」（那是我们自己没接上，不是腾讯刁难）。完成检测用「cookie store 里出现了新 cookie」（不猜 DOM），基线的取法有个坑：**必须在首次 `onPageFinished` 之后才取**，否则验证页一进来写的会话 cookie 会被误判成「验证完成」，拿一份没验证过的 cookie 去重试 → 被再次要求验证的死循环；另加 2 次轮次上限兜底。**明确不做**：不去伪造/获取设备指纹（QIMEI）降低风控等级 —— 合成的 `qimei36` 很可能就是诱因之一，但「不模拟设备指纹、不绕过风控」是本项目红线。**真机已验证**：v2.1.0 → v2.1.2 在三台设备上覆盖升级（release 对 release、debug 对 debug，签名一致、权限仍 11 条），冷启动无崩溃；**S6 上打开扫码浮层连续 9 次 `ptqrlogin http=200 body=ptuiCB('66',…)`**（修复前是连续 403）；播放页音源角标与设置页新入口都在真机 UI 层级里读到。单测 528 全绿。⚠️ **真人扫码那一跳、手机号登录的成功路径与验证码端到端**仍未验证（前者需要真人扫码、后者需要真实短信）** |
| **v2.1.3** | **2026-09-25** | **修用户当场报出的文案 bug：短信验证码登录失败时显示「扫码登录失败」。**根因是**跨功能文案复用** —— 手机号登录的三个失败分支都抓了扫码那条链路的 `sourceQrFailed`，而短信登录与扫码登录是两条完全不同的链路，那句话在任何情况下都是错的（教训：兜底分支最容易随手抓一个现成的字符串，跨功能文案一律不要复用）。同时 `classifyLogin` 不再是「一律 else」：`20271`/`20261`→验证码错、`20277`/`20278`/`20450`→账号受限、`20279`→设备数超限、`104604`→过于频繁、**认不出的码一律中性作答**（上一版就是猜错原因才说出「扫码登录失败」这种话）；手机号登录有了自己的 5 条失败文案（8 语言）。**真机日志同时证明了图形验证码那条路是通的**：`20276`（`c.y.qq.com` 验证页）→ 用户过验证 → **`req.code=0`，短信真的发出去了**；坏掉的只是「失败之后说什么」。单测 530 全绿。（v2.1.1 / v2.1.2 的 tag 保留为历史检查点，均未发布，内容全在本版。）** |
