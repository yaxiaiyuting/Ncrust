...
- **离线缓存**（v1.6.0 Phase 1）：**已经播放过**的音频流按 media3 `SimpleCache` / `CacheDataSource` 语义自动落盘 `filesDir/offline/audio`（上限 512 MiB，LRU），断网时可回放；未播放过的歌不产生任何本地文件。**目前没有显式「下载」入口与离线曲目管理**（Phase 2 未做）
